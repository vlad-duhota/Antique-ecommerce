...
use OctolizeShippingCostOnProductPageVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use OctolizeShippingCostOnProductPageVendor\WPDesk\Logger\WC\WooCommerceCapture;
use OctolizeShippingCostOnProductPageVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \OctolizeShippingCostOnProductPageVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\OctolizeShippingCostOnProductPageVendor\Monolog\Registry::hasLogger($name)) {
            \OctolizeShippingCostOnProductPageVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\OctolizeShippingCostOnProductPageVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \OctolizeShippingCostOnProductPageVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \OctolizeShippingCostOnProductPageVendor\Monolog\Logger($name);
        }
        if (\OctolizeShippingCostOnProductPageVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \OctolizeShippingCostOnProductPageVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\OctolizeShippingCostOnProductPageVendor\Monolog\Logger $logger)
    {
        if (!\defined('OctolizeShippingCostOnProductPageVendor\\WC_LOG_THRESHOLD')) {
            \define('OctolizeShippingCostOnProductPageVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \OctolizeShippingCostOnProductPageVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \OctolizeShippingCostOnProductPageVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \OctolizeShippingCostOnProductPageVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \OctolizeShippingCostOnProductPageVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\OctolizeShippingCostOnProductPageVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \OctolizeShippingCostOnProductPageVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \OctolizeShippingCostOnProductPageVendor\Monolog\Registry::hasLogger($name);
    }
}
