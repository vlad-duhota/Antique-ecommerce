...
namespace OctolizeShippingCostOnProductPageVendor\WPDesk\View;

use OctolizeShippingCostOnProductPageVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use OctolizeShippingCostOnProductPageVendor\WPDesk\View\Resolver\ChainResolver;
use OctolizeShippingCostOnProductPageVendor\WPDesk\View\Resolver\DirResolver;
use OctolizeShippingCostOnProductPageVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \OctolizeShippingCostOnProductPageVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \OctolizeShippingCostOnProductPageVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \OctolizeShippingCostOnProductPageVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \OctolizeShippingCostOnProductPageVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
