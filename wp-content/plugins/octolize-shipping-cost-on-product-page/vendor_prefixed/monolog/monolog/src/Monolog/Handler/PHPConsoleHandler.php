...
use Exception;
use OctolizeShippingCostOnProductPageVendor\Monolog\Formatter\LineFormatter;
use OctolizeShippingCostOnProductPageVendor\Monolog\Logger;
use OctolizeShippingCostOnProductPageVendor\Monolog\Utils;
use OctolizeShippingCostOnProductPageVendor\PhpConsole\Connector;
use OctolizeShippingCostOnProductPageVendor\PhpConsole\Handler;
use OctolizeShippingCostOnProductPageVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \OctolizeShippingCostOnProductPageVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \OctolizeShippingCostOnProductPageVendor\PhpConsole\Connector $connector = null, $level = \OctolizeShippingCostOnProductPageVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('OctolizeShippingCostOnProductPageVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\OctolizeShippingCostOnProductPageVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \OctolizeShippingCostOnProductPageVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \OctolizeShippingCostOnProductPageVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\OctolizeShippingCostOnProductPageVendor\PhpConsole\Helper::isRegistered()) {
            \OctolizeShippingCostOnProductPageVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \OctolizeShippingCostOnProductPageVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \OctolizeShippingCostOnProductPageVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \OctolizeShippingCostOnProductPageVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \OctolizeShippingCostOnProductPageVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
