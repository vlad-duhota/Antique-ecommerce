...
use OctolizeShippingCostOnProductPageVendor\Monolog\Handler\AbstractHandler;
/**
...
class ErrorHandler
{
    ...
    {
        $this->logger->log($this->uncaughtExceptionLevel === null ? \Psr\Log\LogLevel::ERROR : $this->uncaughtExceptionLevel, \sprintf('Uncaught Exception %s: "%s" at %s line %s', \OctolizeShippingCostOnProductPageVendor\Monolog\Utils::getClass($e), $e->getMessage(), $e->getFile(), $e->getLine()), array('exception' => $e));
    ...
    public function handleFatalError()
    {
        ...
        if ($lastError && \in_array($lastError['type'], self::$fatalErrors, \true)) {
            ...
            if ($this->logger instanceof \OctolizeShippingCostOnProductPageVendor\Monolog\Logger) {
                foreach ($this->logger->getHandlers() as $handler) {
                    if ($handler instanceof \OctolizeShippingCostOnProductPageVendor\Monolog\Handler\AbstractHandler) {
                        $handler->close();
                    }
                }
            }
