...
class Xirki_Autoload {
	...
	protected function autoload( $class_name ) {
		...
		// Not a Xirki file, early exit.
		if ( 0 !== stripos( $class_name, 'Xirki' ) ) {
			return;
		}
	...
	protected function get_paths( $class_name ) {
		...
		$name_parts = explode( '_', str_replace( 'Xirki_', '', $class_name ) );

		// Handle modules loading.
		if ( isset( $name_parts[0] ) && 'Modules' === $name_parts[0] ) {
			$path    = dirname( __FILE__ ) . '/modules/';
			$path   .= strtolower( str_replace( '_', '-', str_replace( 'Xirki_Modules_', '', $class_name ) ) ) . '/';
			$paths[] = $path . $filename;
		} elseif ( 'Xirki_Fonts' === $class_name ) {
			$paths[] = dirname( __FILE__ ) . '/modules/webfonts/class-xirki-fonts.php';
		}
		...
		$substr   = str_replace( 'Xirki_', '', $class_name );
