 ...
 */
class Xirki_Control_Switch extends Xirki_Control_Base {
		...
		<div class="switch<# if ( data.choices['round'] ) { #> round<# } #>">
			...
			<label class="switch-label" for="switch_{{ data.id }}">
				<span class="switch-on">
					<# data.choices.on = data.choices.on || '<?php esc_html_e( 'On', 'xirki' ); ?>' #>
					{{ data.choices.on }}
				</span>
				<span class="switch-off">
					<# data.choices.off = data.choices.off || '<?php esc_html_e( 'Off', 'xirki' ); ?>' #>
					{{ data.choices.off }}
				</span>
			</label>
		</div>
