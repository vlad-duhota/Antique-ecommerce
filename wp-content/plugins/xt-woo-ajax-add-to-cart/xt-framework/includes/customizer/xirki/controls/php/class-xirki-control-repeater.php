 ...
 * Repeater control
 */
class Xirki_Control_Repeater extends Xirki_Control_Base {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_html__( 'Add new %s', 'xirki' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_html__( 'Select a Page', 'xirki' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function to_json() {
		...
		$this->json['value'] = apply_filters( "xirki_controls_repeater_value_{$this->id}", $this->json['value'] );
	}
	...
	protected function render_content() {
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_html__( 'Limit: %s rows', 'xirki' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-content">
					<# _.each( data, function( field, i ) { #>
						...
						<div class="repeater-field repeater-field-{{{ field.type }}} repeater-field-{{ field.id }}">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="xirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'xirki' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_html_e( 'No Image Selected', 'xirki' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'xirki' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'xirki' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'xirki' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change Image', 'xirki' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add Image', 'xirki' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="xirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'xirki' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_html_e( 'No File Selected', 'xirki' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_html_e( 'Remove', 'xirki' ); ?></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'xirki' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'xirki' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_html_e( 'Change File', 'xirki' ); ?>
										<# } else { #>
											<?php esc_html_e( 'Add File', 'xirki' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'xirki' ); ?></button>
				</div>
			</li>
		</script>
		<?php
