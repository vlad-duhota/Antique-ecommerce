 ...
 * Typography control.
 */
class Xirki_Control_Typography extends Xirki_Control_Base {
	...
	public function to_json() {
		...
		if ( is_array( $this->json['value'] ) ) {
			foreach ( array_keys( $this->json['value'] ) as $key ) {
				...
				// Fix for https://xplodedthemes.com/issues/1405.
		...
		$this->json['show_variants'] = ( true === Xirki_Fonts_Google::$force_load_all_variants ) ? false : true;
	}
		...
		<div class="wrapper">
			...
			<# if ( ! _.isUndefined( data.default['font-family'] ) ) { #>
				...
				<div class="font-family">
					<h5><?php esc_html_e( 'Font Family', 'xirki' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="xirki-typography-font-family-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'xirki' ); ?>"></select>
				</div>
				<# if ( ! _.isUndefined( data.choices['font-backup'] ) && true === data.choices['font-backup'] ) { #>
					<div class="font-backup hide-on-standard-fonts xirki-font-backup-wrapper">
						<h5><?php esc_html_e( 'Backup Font', 'xirki' ); ?></h5>
						<select {{{ data.inputAttrs }}} id="xirki-typography-font-backup-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'xirki' ); ?>"></select>
					</div>
				<# } #>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant xirki-variant-wrapper">
						<h5><?php esc_html_e( 'Variant', 'xirki' ); ?></h5>
						<select {{{ data.inputAttrs }}} class="variant" id="xirki-typography-variant-{{{ data.id }}}"></select>
					</div>
				<# } #>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['font-size'] ) ) { #>
				...
				<div class="font-size">
					<h5><?php esc_html_e( 'Font Size', 'xirki' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['line-height'] ) ) { #>
				...
				<div class="line-height">
					<h5><?php esc_html_e( 'Line Height', 'xirki' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['letter-spacing'] ) ) { #>
				...
				<div class="letter-spacing">
					<h5><?php esc_html_e( 'Letter Spacing', 'xirki' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['word-spacing'] ) ) { #>
				...
				<div class="word-spacing">
					<h5><?php esc_html_e( 'Word Spacing', 'xirki' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['text-align'] ) ) { #>
				...
				<div class="text-align">
					<h5><?php esc_html_e( 'Text Align', 'xirki' ); ?></h5>
					<div class="text-align-choices">
						<input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Inherit', 'xirki' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Left', 'xirki' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Center', 'xirki' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Right', 'xirki' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Justify', 'xirki' ); ?></span>
							</label>
						</input>
					</div>
				</div>
			...
			<# if ( ! _.isUndefined( data.default['text-transform'] ) ) { #>
				...
				<div class="text-transform">
					<h5><?php esc_html_e( 'Text Transform', 'xirki' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="xirki-typography-text-transform-{{{ data.id }}}">
						<option value=""<# if ( '' === data.value['text-transform'] ) { #>selected<# } #>></option>
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'xirki' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Capitalize', 'xirki' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Uppercase', 'xirki' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Lowercase', 'xirki' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'xirki' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'xirki' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['text-decoration'] ) ) { #>
				...
				<div class="text-decoration">
					<h5><?php esc_html_e( 'Text Decoration', 'xirki' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="xirki-typography-text-decoration-{{{ data.id }}}">
						<option value=""<# if ( '' === data.value['text-decoration'] ) { #>selected<# } #>></option>
						<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'xirki' ); ?></option>
						<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Underline', 'xirki' ); ?></option>
						<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Overline', 'xirki' ); ?></option>
						<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Line-Through', 'xirki' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'xirki' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'xirki' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['margin-top'] ) ) { #>
				...
				<div class="margin-top">
					<h5><?php esc_html_e( 'Margin Top', 'xirki' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['margin-bottom'] ) ) { #>
				...
				<div class="margin-bottom">
					<h5><?php esc_html_e( 'Margin Bottom', 'xirki' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['color'] ) && false !== data.default['color'] ) { #>
				...
				<div class="color">
					<h5><?php esc_html_e( 'Color', 'xirki' ); ?></h5>
					<input {{{ data.inputAttrs }}} type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default['color'] }}" value="{{ data.value['color'] }}" class="xirki-color-control"/>
				</div>
			<# } #>
	...
	protected function format_variants_array( $variants ) {
		$all_variants   = Xirki_Fonts::get_all_variants();
