...
class Xirki_Util {
	...
	/**
	 * Determine if Xirki is installed as a plugin.
	 *
	 * @static
	 * @access public
	 * @since 3.0.0
	...
	public static function is_plugin() {
		...
		foreach ( $plugins as $plugin => $args ) {
			if ( ! $is_plugin && isset( $args['Name'] ) && ( 'Xirki' === $args['Name'] || 'Xirki Toolkit' === $args['Name'] ) ) {
				$is_plugin = true;
				$_plugin   = $plugin;
			}
		}

		// No need to proceed any further if Xirki wasn't found in the list of plugins.
		if ( ! $is_plugin ) {
			return false;
		}
	...
	public static function get_variables() {
			...
			if ( isset( $field['variables'] ) && $field['variables'] && ! empty( $field['variables'] ) ) {
				...
				foreach ( $field['variables'] as $field_variable ) {
					...
					if ( isset( $field_variable['name'] ) ) {
						...
						$variables[ $field_variable['name'] ] = Xirki_Values::get_value( $field['settings'] );
						if ( $variable_callback ) {
							$variables[ $field_variable['name'] ] = call_user_func( $field_variable['callback'], Xirki_Values::get_value( $field['settings'] ) );
						}
					}
				}
			}
		...
		// Pass the variables through a filter ('xirki_variable') and return the array of variables.
		return apply_filters( 'xirki_variable', $variables );
	}
	...
	public function http_request( $request = array(), $url = '' ) {
		...
		foreach ( $plugins['plugins'] as $plugin ) {
			if ( isset( $plugin['Name'] ) && 'Xirki Toolkit' === $plugin['Name'] ) {
				$exists = true;
			}
		}
		// Inject data.
		if ( ! $exists && defined( 'XIRKI_PLUGIN_FILE' ) ) {
			$plugins['plugins']['xirki/xirki.php'] = get_plugin_data( XIRKI_PLUGIN_FILE );
		}
