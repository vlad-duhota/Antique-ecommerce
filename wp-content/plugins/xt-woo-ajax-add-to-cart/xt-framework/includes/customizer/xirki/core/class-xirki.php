 ...
 * For documentation please see https://xplodedthemes.com/wiki
 *
 * @package     Xirki
 * @category    Core
 ...
 */
class Xirki extends Xirki_Init {

	/**
	 * Absolute path to the Xirki folder.
	 *
	 * @static
	 * @access public
	 * @var string
	...
	public static function get_option( $config_id = '', $field_id = '' ) {
		return Xirki_Values::get_value( $config_id, $field_id );
	}
	...
	public static function add_config( $config_id, $args = array() ) {
		$config                             = Xirki_Config::get_instance( $config_id, $args );
	...
	public static function add_panel( $id = '', $args = array() ) {
		...
		if ( false === strpos( $args['type'], 'xirki-' ) ) {
			$args['type'] = 'xirki-' . $args['type'];
		}
		self::$panels[ $id ] = $args;
	}
	...
	public static function add_field( $config_id, $args ) {
		if ( doing_action( 'customize_register' ) ) {
			_doing_it_wrong( __METHOD__, esc_html__( 'Xirki fields should not be added on customize_register. Please add them directly, or on init.', 'xirki' ), '3.0.10' );
		}
		...
		$classname = 'Xirki_Field_' . str_replace( ' ', '_', ucwords( $str ) );
		if ( class_exists( $classname ) ) {
			new $classname( $config_id, $args );
			return;
		}
		if ( false !== strpos( $classname, 'Xirki_Field_Xirki_' ) ) {
			$classname = str_replace( 'Xirki_Field_Xirki_', 'Xirki_Field_', $classname );
			if ( class_exists( $classname ) ) {
				new $classname( $config_id, $args );
				return;
			}
		}
		new Xirki_Field( $config_id, $args );
	}
