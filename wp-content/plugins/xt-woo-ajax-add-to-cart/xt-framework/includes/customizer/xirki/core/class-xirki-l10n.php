...
class Xirki_L10n {
	...
	public function __construct() {
		...
		// If Xirki is installed as a plugin, load the texdomain.
		if ( Xirki_Util::is_plugin() ) {
			add_action( 'plugins_loaded', array( $this, 'load_textdomain' ) );
			return;
		}

		// If we got this far, then Xirki is embedded in a plugin.
	...
	public function load_textdomain() {
		...
		load_plugin_textdomain( $this->textdomain, false, Xirki::$path . '/languages' );
	}
	...
	protected function get_paths() {
		return array(
			...
			Xirki::$path . '/languages/' . $this->textdomain . '-' . get_locale() . '.mo',
		);
	}

	/**
	 * Allows overriding the "xirki" textdomain from a theme.
	 *
	 * @since 3.0.12
	 * @access public
	...
	public function override_load_textdomain( $override, $domain, $mofile ) {
		...
		if ( isset( $l10n[ $this->get_theme_textdomain() ] ) ) {
			$l10n['xirki'] = $l10n[ $this->get_theme_textdomain() ]; // phpcs:ignore WordPress.WP.GlobalVariablesOverride
		}

		// Check if the domain is "xirki".
		if ( 'xirki' === $domain ) {
			return true;
		}
		return $override;
