...
class Xirki_Field {
	...
	/**
	 * The ID of the xirki_config we're using.
	 *
	 * @see Xirki_Config
	 * @access protected
	 * @var string
	 */
	protected $xirki_config = 'global';
	 ...
	 *
	 * @see https://xplodedthemes.com/docs/arguments/output
	 * @access protected
	 * @var array
	 */
	 ...
	 *
	 * @see https://xplodedthemes.com/docs/arguments/js_vars
	 * @access protected
	 * @var array
	 */
	 ...
	 *
	 * @see https://xplodedthemes.com/docs/arguments/variables
	 * @access protected
	 * @var array
	 */
	 ...
	 * Then it adds the field to Xirki::$fields.
	 *
	 * @access public
	 * @param string $config_id    The ID of the config we want to use.
	 *                             Defaults to "global".
	 *                             Configs are handled by the Xirki_Config class.
	...
	public function __construct( $config_id = 'global', $args = array() ) {
		...
		if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Typo found in field %s - setting instead of settings.', 'xirki' ), esc_html( $args['settings'] ) ), '3.0.10' );
		...
		if ( is_array( $config_id ) && empty( $args ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'xirki' ), esc_html( $args['settings'] ), 'https://xplodedthemes.com/docs/getting-started/fields.html' ), '3.0.10' );
			$args      = $config_id;
			$config_id = 'global';
		}

		$args['xirki_config'] = $config_id;

		$this->xirki_config = $config_id;
		if ( '' === $config_id ) {
			/* translators: %1$s represents the field ID where the error occurs. %2$s is the URL in the documentation site. */
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'Config not defined for field %1$s - See %2$s for details on how to properly add fields.', 'xirki' ), esc_html( $args['settings'] ), 'https://xplodedthemes.com/docs/getting-started/fields.html' ), '3.0.10' );
			$this->xirki_config = 'global';
		}
		...
		$config_defaults = ( isset( Xirki::$config['global'] ) ) ? Xirki::$config['global'] : array();
		if ( 'global' !== $this->xirki_config && isset( Xirki::$config[ $this->xirki_config ] ) ) {
			$config_defaults = Xirki::$config[ $this->xirki_config ];
		}
	...
	protected function set_field() {
		...
		Xirki::$fields[ $this->settings ] = $args;
	...
	protected function set_option_type() {
		...
		if ( 'theme_mods' === $this->option_type ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html( 'Typo found in field %s - "theme_mods" vs "theme_mod"', 'xirki' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_partial_refresh() {
		...
		foreach ( $this->partial_refresh as $id => $args ) {
			if ( ! is_array( $args ) || ! isset( $args['selector'] ) || ! isset( $args['render_callback'] ) || ! is_callable( $args['render_callback'] ) ) {
				/* translators: %1$s represents the field ID where the error occurs. */
				_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"partial_refresh" invalid entry in field %s', 'xirki' ), esc_html( $this->settings ) ), '3.0.10' );
	...
	protected function set_settings() {
		...
		if ( ! is_array( $this->settings ) ) {
			$this->settings = array(
				'xirki_placeholder_setting' => $this->settings,
			);
		}
		$settings = array();
		...
		if ( isset( $this->settings['xirki_placeholder_setting'] ) ) {
			$this->settings = $this->settings['xirki_placeholder_setting'];
		}
	}
	...
	protected function set_active_callback() {
		if ( is_array( $this->active_callback ) ) {
			if ( ! is_callable( $this->active_callback ) ) {
				...
				// Bugfix for https://xplodedthemes.com/issues/1961.
	...
	protected function set_output() {
		...
		if ( ! is_array( $this->output ) ) {
			...
			_doing_it_wrong( __METHOD__, sprintf( esc_html__( '"output" invalid format in field %s. The "output" argument should be defined as an array of arrays.', 'xirki' ), esc_html( $this->settings ) ), '3.0.10' );
			$this->output = array(
				array(
					'element' => $this->output,
				),
		...
		foreach ( $this->output as $key => $output ) {
			...
			// Fix for https://xplodedthemes.com/issues/1659#issuecomment-346229751.
