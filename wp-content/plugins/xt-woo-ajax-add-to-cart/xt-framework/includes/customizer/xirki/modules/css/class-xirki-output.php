...
class Xirki_Output {
	...
	/**
	 * The Xirki configuration used in the field.
	 *
	 * @access protected
	 * @var string
	 */
	...
	protected function apply_pattern_replace( $output, $value ) {
		if ( isset( $output['pattern_replace'] ) && is_array( $output['pattern_replace'] ) ) {
			$option_type = ( '' !== Xirki::get_config_param( $this->config_id, 'option_type' ) ) ? Xirki::get_config_param( $this->config_id, 'option_type' ) : 'theme_mod';
			$option_name = Xirki::get_config_param( $this->config_id, 'option_name' );
			$options     = array();
			if ( $option_name ) {
			...
			foreach ( $output['pattern_replace'] as $search => $replace ) {
				...
				switch ( $option_type ) {
					...
					default:
						...
						if ( ! $replacement ) {
							$replacement = Xirki_Values::get_value( $this->field['xirki_config'], $replace );
						}
				}
			...
			if ( isset( $output['exclude'] ) && is_array( $output['exclude'] ) ) {
				foreach ( $output['exclude'] as $exclude ) {
					if ( is_array( $value ) ) {
						...
						// Fixes https://xplodedthemes.com/issues/1416.
	...
	protected function process_property_value( $property, $value ) {
		$properties = apply_filters(
			"xirki_{$this->config_id}_output_property_classnames",
			array(
				'font-family'         => 'Xirki_Output_Property_Font_Family',
				'background-image'    => 'Xirki_Output_Property_Background_Image',
				'background-position' => 'Xirki_Output_Property_Background_Position',
			)
		);
