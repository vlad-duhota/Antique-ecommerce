<?php
/**
 * Telemetry implementation for Xirki.
 *
 * @package     Xirki
 * @category    Core
 ...
 */
final class Xirki_Modules_Telemetry {
	...
	protected function __construct() {
		...
		if ( ! apply_filters( 'xirki_telemetry', true ) ) {
			return;
		}
	...
	public function maybe_send_data() {
		...
		if ( ! get_option( 'xirki_telemetry_optin' ) ) {
			return;
		}
		...
		$sent = get_option( 'xirki_telemetry_sent' );
		if ( ! $sent || $sent < time() - MONTH_IN_SECONDS ) {
			$this->send_data();
			update_option( 'xirki_telemetry_sent', time() );
		}
	}
	...
	private function send_data() {
		...
		// Ping remote server.
		wp_remote_post(
			'https://wplemon.com/?action=xirki-stats',
			array(
				'method'   => 'POST',
				'blocking' => false,
				'body'     => array_merge(
	...
	public function admin_notice() {
		...
		$data = $this->get_data();
		?>
		<div class="notice notice-info xirki-telemetry">
			<h3><strong><?php esc_html_e( 'Help us improve Xirki.', 'xirki' ); ?></strong></h3>
			<p style="max-width: 76em;"><?php _e( 'Help us begin a dialogue with theme developers, collaborate and improve both the theme you are using and the Xirki framework by agreeing to send anonymous data. <strong>The data is completely anonymous and we will never collect any identifyable information about you or your website.</strong>', 'xirki' ); // phpcs:ignore WordPress.Security.EscapeOutput ?></p>
			<table class="data-to-send hidden">
				<thead>
					<tr>
						<th colspan="2"><?php esc_html_e( 'Data that will be sent', 'xirki' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
						<td style="min-width: 200px;"><?php esc_html_e( 'PHP Version', 'xirki' ); ?></td>
						<td><code><?php echo esc_html( $data['phpVer'] ); ?></code></td>
					</tr>
					<tr>
						<td><?php esc_html_e( 'Theme Name', 'xirki' ); ?></td>
						<td><code><?php echo esc_html( $data['themeName'] ); ?></code></td>
					</tr>
					<tr>
						<td><?php esc_html_e( 'Theme Author', 'xirki' ); ?></td>
						<td><code><?php echo esc_html( $data['themeAuthor'] ); ?></code></td>
					</tr>
					<tr>
						<td><?php esc_html_e( 'Theme URI', 'xirki' ); ?></td>
						<td><code><?php echo esc_html( $data['themeURI'] ); ?></code></td>
					</tr>
					<tr>
						<td><?php esc_html_e( 'Field Types Used', 'xirki' ); ?></td>
					...
					<tr>
						<th colspan="2">
							...
							printf(
								...
								__( 'We believe in complete transparency. You can see the code used on our server <a href="%1$s" rel="nofollow">here</a>, and the results of the statistics we\'re gathering on <a href="%2$s" rel="nofollow">this page</a>.', 'xirki' ), // phpcs:ignore WordPress.Security.EscapeOutput
								'https://xplodedthemes.com-telemetry-server',
								'https://wplemon.com/xirki-telemetry-statistics/'
							);
							?>
						</th>
					</tr>
