 ...
 * Handles CSS output.
 */
final class Xirki_Modules_CSS_Generator {
	...
	private function __construct() {
		if ( is_null( self::$google_fonts ) ) {
			self::$google_fonts = Xirki_Fonts::get_google_fonts();
		}
	}
	...
	public static function css( $field ) {
		...
		self::$value = Xirki_Values::get_sanitized_field_value( $field );

		// Find the class that will handle the outpout for this field.
		$classname            = 'Xirki_Output';
		$default_classnames   = array(
			'xirki-background' => 'Xirki_Output_Field_Background',
			'xirki-dimensions' => 'Xirki_Output_Field_Dimensions',
			'xirki-image'      => 'Xirki_Output_Field_Image',
			'xirki-typography' => 'Xirki_Output_Field_Typography',
			'xirki-multicolor' => 'Xirki_Output_Field_Multicolor',
		);
		$field_output_classes = apply_filters( 'xirki_output_control_classnames', $default_classnames );
		$field_output_classes = apply_filters( "xirki_{$field['xirki_config']}_output_control_classnames", $field_output_classes );
		if ( array_key_exists( self::$field_type, $field_output_classes ) ) {
			$classname = $field_output_classes[ self::$field_type ];
		}
		$obj = new $classname( $field['xirki_config'], self::$output, self::$value, $field );
	...
	public static function styles_parse( $css = array() ) {
		...
		// Pass our styles from the xirki_styles_array filter.
		$css = apply_filters( 'xirki_styles_array', $css );
