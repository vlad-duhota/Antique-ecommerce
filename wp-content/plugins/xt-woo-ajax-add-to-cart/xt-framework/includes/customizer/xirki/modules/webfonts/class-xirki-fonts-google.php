 ...
 */
final class Xirki_Fonts_Google {

	/**
	 * The Xirki_Fonts_Google instance.
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_xirki_fonts_google_all_get', array( $this, 'get_googlefonts_json' ) );
		add_action( 'wp_ajax_nopriv_xirki_fonts_google_all_get', array( $this, 'get_googlefonts_json' ) );
		add_action( 'wp_ajax_xirki_fonts_standard_all_get', array( $this, 'get_standardfonts_json' ) );
		add_action( 'wp_ajax_nopriv_xirki_fonts_standard_all_get', array( $this, 'get_standardfonts_json' ) );

		// Populate the array of google fonts.
		$this->google_fonts = Xirki_Fonts::get_google_fonts();
	}
	 ...
	 *
	 * @return object Xirki_Fonts_Google
	 */
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new Xirki_Fonts_Google();
		}
	...
	public function generate_google_font( $args ) {
		...
		if ( isset( $args['type'] ) && 'xirki-typography' === $args['type'] ) {

			// Get the value.
			$value = Xirki_Values::get_sanitized_field_value( $args );
			...
			if ( ! isset( $value['font-family'] ) || ! Xirki_Fonts::is_google_font( $value['font-family'] ) ) {
				return;
			}
	...
	public function get_standardfonts_json() {
		echo wp_json_encode( Xirki_Fonts::get_standard_fonts() );
		wp_die();
	}
}
