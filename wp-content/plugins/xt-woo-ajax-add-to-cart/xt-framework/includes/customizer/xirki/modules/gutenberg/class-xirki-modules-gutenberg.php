<?php
/**
 * Gutenberg integration for Xirki.
 *
 * This class contains methods for integrating Xirki with
...
class Xirki_Modules_Gutenberg {
	...
	public function get_styles() {
		...
		foreach ( $this->configs as $config_id => $args ) {
			...
			$styles      = apply_filters( "xirki_gutenberg_{$config_id}_dynamic_css", $styles );
	 ...
	 *
	 * Feature can be disabled by XIRKI_NO_OUTPUT constant,
	...
	private function is_disabled( $args = array() ) {
		if ( defined( 'XIRKI_NO_OUTPUT' ) && true === XIRKI_NO_OUTPUT ) {
			return true;
		}
	...
	private function set_modules_css() {
		$this->modules_css = Xirki_Modules_CSS::get_instance();
	}
	...
	private function set_google_fonts() {
		$this->google_fonts = Xirki_Fonts_Google::get_instance();
	}
	...
	private function set_modules_webfonts() {
		$this->modules_webfonts = Xirki_Modules_Webfonts::get_instance();
	}
}
