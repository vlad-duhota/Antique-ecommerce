...
<div class="product__right"><?php
	if ( $price_html = $product->get_price_html() ){ ?>
		<p class="price"><?php echo $price_html; ?></p><?php
	}

	if( in_array( $product->get_ID(), array_column( WC()->cart->get_cart(), 'product_id' ) ) ) { ?>
		<a class="add_to_cart_button" href="<?php echo get_home_url()?>/cart">View Cart</a><?php
	} else{
		echo apply_filters(
    ...
    if ($availability['availability']) {
		echo apply_filters('woocommerce_stock_html',
			'<p class="add_to_cart_button '.esc_attr($availability['class']).'">'.esc_html($availability['availability']).'</p>',
